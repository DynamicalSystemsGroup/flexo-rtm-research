...
- **Typed simplicial complex framing (Zargham 2026)** — *one related research line; not on `flexo-rtm`'s roadmap*. Per [[ADR-032 Methodology Agnosticism as Foundational Axiom]], `flexo-rtm`'s v0.1 vocabulary (typed attestations, aspects, named approvers) aligns with this research line as forward-compatible interop, not as a commitment to its eventual destination; adopters who choose to run a topological audit as one possible downstream-analysis mode can do so without translation, and adopters who don't are unaffected. See [[Topological Framework Future Work]].
- **TDA capability** — tracks the topological research line, not `flexo-rtm`'s roadmap; relevant only to adopters running topological downstream analysis.
