...
| `rtm:hasContentHash` | `xsd:string` | Algorithm-prefixed content hash, e.g., `sha256:abcd1234...`. Canonical content addressing for artifact bytes. |
...
- **`sha256:` content hashes.** Any storage layer — filesystem, S3, HTTP, IPFS, git-LFS, DVC, Datalad, Nix store — produces the same hash for the same bytes. The hash is the identity; storage is interchangeable.
