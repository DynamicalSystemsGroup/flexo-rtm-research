...
## Optional lifecycle-stage scope metadata

Scopes MAY carry **optional** lifecycle-stage metadata via the `rtm:lifecycleStage` predicate. The property's range is `skos:Concept`, so adopters can declare any SKOS concept scheme they use — INCOSE / ISO/IEC/IEEE 15288 stages are **one example** vocabulary (the optional `ontology/lifecycle/incose.ttl` module ships the six canonical IRIs); programs using DO-178C DAL gates, NASA Phase A–F, ISO 9001 process gates, Agile sprint cycles, MIL-STD-498 phasing, or customer-specific milestone vocabularies declare their own. The framework is **methodology-neutral**; no lifecycle vocabulary is privileged, and `rtm:lifecycleStage` itself is optional. See [[Engineering Lifecycle Stages]] for the optional pattern and [[ADR-029 Engineering Lifecycle Stages as Scope Metadata]] for the locked decision.

The lifecycle stage is **scope-level metadata**, not a property of individual triples within the scope; the same scope-as-first-class-RDF discipline applies. Per [[Storage Layer Flexo Conventions]] F4, lifecycle stage MAY be captured in commit metadata alongside the scope IRI for adopters who use a lifecycle vocabulary. There is no scope-level state machine in core, no stage-aware gate-dispatch profile shipped, and no required SHACL on lifecycle. Adopters who want stage-aware gate relaxation build their own adopter-specific profiles on the substrate (`rtm:lifecycleStage` property, scope-as-first-class-RDF, gap-code vocabulary).

**Regression handling is at the attestation level**, not at the scope level. When upstream changes invalidate a downstream attestation, the affected attestation is marked `rtm:status/deprecated` with `prov:wasInvalidatedBy` recording the cause; the cert artifact surfaces deprecated attestations as **T9** gaps per [[Gap Taxonomy]]. This mechanism is methodology-neutral and does not require any scope-level lifecycle tracking. See [[ADR-031 Attestation Status Pass Fail Deferred Deprecated]] for the locked decision.
...
- [[Engineering Lifecycle Stages]] — optional `rtm:lifecycleStage` scope metadata; methodology-neutral, INCOSE / ISO 15288 as one example
- [[ADR-031 Attestation Status Pass Fail Deferred Deprecated]] — methodology-neutral regression handling at the attestation level, replacing the earlier scope-level state-machine framing
