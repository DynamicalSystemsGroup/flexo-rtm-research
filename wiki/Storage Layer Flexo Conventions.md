...
Commit metadata also captures the active **lifecycle stage** of the scope (`rtm:lifecycleStage`) alongside the scope IRI. The lifecycle vocabulary ships in v0.1 for forward-compat data accumulation; the v0.2 oracle will use this captured stage to drive **stage-aware gates** — relaxing T1/T2 to informational at the Concept stage, triggering auto-rerun via [[Transcript Replay Semantics]] when a change invalidates prior certifications, prompting re-attestation when a fall-back transition is recorded. v0.1 adopters tagging scopes today are accumulating data the v0.2 oracle will activate without rework. See [[Engineering Lifecycle Stages]] for the full vocabulary, the six canonical INCOSE / ISO 15288 stage IRIs, the lifecycle state machine, and the v0.1-vs-v0.2 split.
