...
Commit metadata also captures the active **lifecycle stage** of the scope (`rtm:lifecycleStage`) alongside the scope IRI. The lifecycle vocabulary ships in v0.1 for forward-compat data accumulation; the v0.2 oracle will use this captured stage to drive **stage-aware gates** — relaxing T1/T2 to informational at exploratory or concept stages, triggering auto-rerun via [[Transcript Replay Semantics]] when a change invalidates prior certifications, prompting re-attestation when a fall-back transition is recorded. v0.1 adopters tagging scopes today are accumulating data the v0.2 oracle will activate without rework. See [[Engineering Lifecycle Stages]] for the full vocabulary, the seven INCOSE-aligned stage IRIs, the lifecycle state machine, and the v0.1-vs-v0.2 split.
...
- [[Engineering Lifecycle Stages]] — `rtm:lifecycleStage` captured in commit metadata alongside scope IRI; v0.2 stage-aware gate dispatch reads it.
