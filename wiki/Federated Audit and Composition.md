...
In practice, large institutional systems are not certified by one team alone. They are **composed** from many self-certified scopes, each of which is then **audited** by additional parties — peer reproducibility auditors, qualified third-party hosts, sister-organization verifiers, regulatory reviewers, accredited test labs. Each additional party adds a signature that reinforces (or contradicts) the underlying self-certification. The composed system-of-interest is itself a named-graph patchwork of scopes, and the **adequacy** of that patchwork (does it cover the system-of-interest?) and the **sufficiency** of its signers (are enough qualified orgs attesting?) are first-class certification questions at the composition scale.
