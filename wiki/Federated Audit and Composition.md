...
- **Closed-triangle aggregation across federated audits**. If the topological research line matures into an applied audit (per [[ADR-032 Methodology Agnosticism as Foundational Axiom]] — it is one possible downstream-analysis path, not `flexo-rtm`'s destination), its closed-triangle audit would operate over the union of self-cert + federated-audit attestations as a downstream-analysis mode; `flexo-rtm` does not perform the closure, and adopters running other downstream paths (SLSA, GSN, ARP4754A, in-house) consume the same data with their own analyses.
...
The topological framework articulated in Zargham (2026) is one possible downstream-analysis path that consumes `flexo-rtm`'s data — **not `flexo-rtm`'s destination** (per [[ADR-032 Methodology Agnosticism as Foundational Axiom]] and [[Topological Framework Future Work]]). If the research line matures and an adopter chooses to run the closed-triangle audit as a downstream mode, federated audit attestations slot in naturally — they are named-approver attestations such an audit could require on validation edges or on assurance-face closure at the composed-model scale. The same named-approver discipline ([[ADR-021 Three Attestation Subclasses Ship in v0.1]]) and identity-projection discipline ([[ADR-024 Identity by Thin Projection of External Sources]]) carry through. Other downstream-analysis paths (SLSA supply-chain audits, GSN assurance-case construction, ARP4754A airborne-systems audits, in-house analysis layers) consume the same data with their own analyses — `flexo-rtm` privileges none.
