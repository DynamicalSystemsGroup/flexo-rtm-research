...
|---|---|
| **Service provider catalogs & discovery** | OSLC catalogs assume a single centralized authoritative tool publishing capabilities; `flexo-rtm` adopts a **polycentric** model in which multiple organizations hold **scoped authorities** (each scope is an Authoritative Source of Truth, ASOT, for its data — see [[ADR-030 Polycentric ASOT Authority Model]]). The v0.1 adapter substitutes **explicit configuration** (adopters declare OSLC sources in `flexo-rtm.yaml`). Live HTTP connectors in v0.2 can speak the catalog protocol against running Doors / Jama / Polarion servers, treating each as one ASOT in the broader composition. |
...
| **Single-RM-database assumption** | OSLC-RM was designed for a world where one server is "the" RM database. `flexo-rtm` adopts the polycentric ASOT pattern from MOSA practice: requirements traceability is distributed across **scoped authorities** held by different organizations (engineering teams, regulatory bodies, qualified auditors), with controlled interfaces between them. Each scope is an ASOT for its content; scopes compose into higher-order scopes and may overlap. The v0.1 adapter operates at the **file/RDF level**; v0.2 live connectors integrate centralized RM tools as additional ASOTs in the composition. |
