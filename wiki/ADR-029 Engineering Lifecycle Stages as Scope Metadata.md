...
Systems do not come into existence ex nihilo. The user's framing in research issue #6 names the engineering reality directly: systems emerge from engineering work that starts with stakeholder framing and requirements specification (the activities the INCOSE Systems Engineering Handbook places inside the Concept stage), then proceeds into development, production, utilization, support, and eventually retirement. New scopes come into existence underspecified and gain detail over time; some scopes reach certification and then return to earlier stages when changes invalidate prior certifications. Per the INCOSE Systems Engineering Handbook and ISO/IEC/IEEE 15288 (see [[INCOSE V2 Review]]), lifecycle stages are a first-class organizing concept with a settled canonical six-stage set — and `flexo-rtm` should be able to assign those exact stages to scopes (named graphs) without inventing new ones (per research issue #8).

The user's framing identifies two direct consequences:

1. **Early-stage models (named graphs) that do not satisfy requirements traceability are not expected to** — gating on T1 (orphan requirement) or T2 (dangling evidence) during Concept-stage work would be noisy and create UX issues.
...
2. **Class** `rtm:LifecycleStage` and the **six canonical INCOSE / ISO 15288 stage IRIs** aligned via `skos:closeMatch` (per the [[INCOSE V2 Review]] adoption pattern): `rtm:stage/concept`, `rtm:stage/development`, `rtm:stage/production`, `rtm:stage/utilization`, `rtm:stage/support`, `rtm:stage/retirement`. The Concept stage explicitly includes the early framing, stakeholder elicitation, and requirements specification activities the INCOSE handbook places inside it; `flexo-rtm` does not introduce parallel sub-stages (no `exploratory`, no `requirements-specification`) — we use existing standards rather than reinvent (per research issue #8).
...
1. **Stage-aware gate relaxation.** Gap codes T1 / T2 / T3 / T4 / T5 / T8 are reported as **informational, not blocking**, at the Concept stage. They all become blocking at Development and remain blocking through Production, Utilization, Support, and Retirement. T6 (failed attestation) and T7 (unapproved attestation, structurally impossible) are unchanged across stages — recorded failure is always recorded failure; unapproved attestation is always SHACL-rejected.
2. **Lifecycle state machine.** Transition rules, fall-back triggers (new or changed requirements → Concept; evidence invalidation → Development; guidance change → Development), auto-rerunnable detection via [[Transcript Replay Semantics]] dispatch, and re-cert prompts in the operational-layer skill.
...
- **Early-stage UX is cleaner.** Concept-stage scopes can be tagged with their actual stage; the v0.2 oracle relaxes traceability gates appropriately. Engineers framing a new problem or eliciting stakeholder requirements don't have to suppress checks they want to keep running for the data they're collecting
...
- **The six-stage set matches INCOSE / ISO 15288 exactly.** No parallel `flexo-rtm`-specific stages compete with the standard; programs already organizing around the INCOSE handbook or ISO/IEC/IEEE 15288 can adopt without translation. Finer-grained internal phasing (concept-of-operations vs. requirements-specification vs. design entry) is tracked through aspects, milestones, or program-specific metadata layered on top, not by minting new top-level stages (per research issue #8)

## Alternatives Considered

- **Ignore lifecycle and treat all scopes the same.** Apply T1–T8 uniformly regardless of stage. **Rejected.** The user's framing is explicit: gating on orphan requirements during Concept-stage work would be noisy and create UX issues. Engineers would either suppress the checks they actually want or develop the bad habit of ignoring the audit report. The lifecycle stage is the mechanism that aligns gate severity with what programs actually want to know at each stage of engineering work.
...
- **Ship lifecycle vocabulary alongside the topological framework only when the framework is ready.** Bundle the two. **Rejected.** Lifecycle stages are useful **today** for tagging scopes, even before the v0.2 mechanism activates. The federated-audit primitive ([[Federated Audit and Composition]], [[ADR-028 Scope-Level Adequacy and Sufficiency for Federated Audit]]) composes with lifecycle stages in v0.1 — for instance, a composition adequacy criterion can require "every constituent scope is at `rtm:stage/development` or later" using only v0.1 vocabulary. Decoupling lifecycle from the framework lets the two extensions land on their own timelines.
- **Introduce parallel `flexo-rtm`-specific sub-stages (e.g., `exploratory` before Concept, `requirements-specification` between Concept and Development).** **Rejected** (per research issue #8). The INCOSE Systems Engineering Handbook and ISO/IEC/IEEE 15288 define a canonical six-stage set; the Concept stage explicitly includes the early framing and requirements-specification activities. Inventing parallel sub-stages would reinvent rather than reuse existing standards, fragment the stage vocabulary across adopters, and create translation friction for programs already organized around the standard. Finer-grained internal phasing is properly tracked through aspects, program milestones, or operational-layer metadata, not by minting new top-level stages.
...
- Closes flexo-rtm-research issue #8 (Don't force exploratory stage) — stage IRI set narrowed to the canonical INCOSE/ISO 15288 six
